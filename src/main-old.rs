...
// --- CONFIG ---
const BASE_SAMPLE_RATE: u32 = 48000;
...
const CQT_HOP_SIZE: usize = (256 * SR_MULT) as usize; 
    ...
    fn new(map_type: ColorMapType) -> Self {
        ...
        for i in 0..=255 {
            ...
            let gradient = match map_type {
                ...
                ColorMapType::Plasma => colorous::TURBO, // Adjusted colorous options
...
// --- OPTIMIZATION: 1D Y-Mapping Shader LUT ---
struct YMappingLut {
    bytes: Vec<u8>,
}

impl YMappingLut {
    fn new(scale_type: ScaleType, is_cqt: bool, max_freq: f32, nyquist: f32) -> Self {
        ...
        for i in 0..1024 {
            ...
            let current_hz = match scale_type {
                ScaleType::Bark => {
                    ...
                    let current_bark = norm_f * bark_max;
                    let z_over_6 = current_bark / 6.0;
                    600.0 * (z_over_6.exp() - (-z_over_6).exp()) / 2.0
                },
                ScaleType::Logarithmic => {
                    let log_min = min_log_freq.log2();
                    let log_max = max_freq.log2();
                    2.0f32.powf(log_min + norm_f * (log_max - log_min))
                },
                ScaleType::Mel => {
                    let mel_max = 2595.0 * (1.0 + max_freq / 700.0).log10();
                    let current_mel = norm_f * mel_max;
            ...
            // 16-bit Precision packed into R and G channels to eliminate all pixel-stepping artifacts
            let mapped_clamp = mapped_y.clamp(0.0, 1.0);
            let val_u16 = (mapped_clamp * 65535.0) as u16;
            let r = (val_u16 >> 8) as u8;
            let g = (val_u16 & 0xFF) as u8;
...
fn create_y_mapping_texture(lut: &YMappingLut) -> Texture2D {
    let img = Image { width: 1024, height: 1, bytes: lut.bytes.clone() };
    let tex = Texture2D::from_image(&img);
    tex.set_filter(FilterMode::Linear);
    tex
}
...
// --- OPTIMIZATION: Extremely Lightweight Fragment Shader ---
// Replaces millions of per-pixel mathematical calculations with a single 1D memory lookup.
...
uniform sampler2D y_mapping;

void main() {
    // 1. Invert Y to map UI 0.0 to visual bottom
    float norm_f = 1.0 - uv.y; 
    
    // 2. Fetch the precalculated mapping coordinates based on the current scale setting
    // The Red and Green channels hold a packed 16-bit float for perfectly smooth precision
    vec4 map_val = texture2D(y_mapping, vec2(norm_f, 0.5));
    float mapped_y = map_val.r + map_val.g / 256.0;
    
    // 3. Sample the audio texture using the mapped coordinate
    float intensity = texture2D(Texture, vec2(uv.x, mapped_y)).r;
...
async fn main() {
    ...
    thread::spawn(move || {
        ...
        let open_stream = |device_name: Option<&String>| -> Option<Simple> {
            ...
            let attr = BufferAttr {
                maxlength: frag_size * 4, // <-- Caps the server backlog to ~60ms to eliminate wake-up latency
        ...
        loop {
            ...
            if let Some(ref s) = stream {
                if let Ok(_) = s.read(&mut buf) {
                    ...
                    // Stream read failed (likely suspended). 
    ...
    thread::spawn(move || {
        ...
        // OPTIMIZATION 1: Track how long the audio has been silent to elegantly put the drawing routine to sleep
            ...
            if pending_buffer.len() < MIN_HOP_SIZE {
                ...
                match audio_rx.recv_timeout(std::time::Duration::from_millis(50)) {
                    Ok(new_data) => {
                        ...
                        consecutive_black_hops = 0; // AUDIO RETURNED! Reset the blackout counter instantly.
                    }
                    Err(std::sync::mpsc::RecvTimeoutError::Timeout) => {
                        ...
                        if elapsed >= cqt_hop_duration {
                            ...
                            // OPTIMIZATION 1: Only push black pixels if we haven't already filled the screen with black
                            let hops_to_push = std::cmp::min(
                                missed_hops, 
                                (MAX_VIEW_LEN as u64).saturating_sub(consecutive_black_hops)
                            );
                            
                            if hops_to_push > 0 {
                                if let Ok(mut layers) = layers_ref.lock() {
                                    for layer in layers.iter_mut() {
                                        for _ in 0..hops_to_push {
    ...
    // Initialize the Y Mapping Texture for the Shader LUT
    let mut current_scale_type = local_settings.scale_type;
    let mut current_is_cqt = last_fft_idx >= RESOLUTIONS.len();
    let mut y_mapping_texture = create_y_mapping_texture(&YMappingLut::new(current_scale_type, current_is_cqt, MAX_DISPLAY_FREQ, SAMPLE_RATE as f32 / 2.0));

    let shader_material = load_material(
        ShaderSource::Glsl { vertex: VERTEX_SHADER, fragment: FRAGMENT_SHADER },
        MaterialParams {
            uniforms: vec![], // Uniforms completely eliminated to save GPU instructions!
            textures: vec!["colormap".to_string(), "y_mapping".to_string()],
            ..Default::default()
        },
    ).unwrap();
    ...
    loop {
        ...
        // Check if the scale settings changed to rebuild the Shader LUT
        let is_cqt_bool = actual_fft_idx >= RESOLUTIONS.len();
        if local_settings.scale_type != current_scale_type || is_cqt_bool != current_is_cqt {
            current_scale_type = local_settings.scale_type;
            current_is_cqt = is_cqt_bool;
            y_mapping_texture = create_y_mapping_texture(&YMappingLut::new(current_scale_type, current_is_cqt, MAX_DISPLAY_FREQ, SAMPLE_RATE as f32 / 2.0));
        }

        // OPTIMIZATION 2: Fast Mutex Scope bypassing double-buffer
        {
            if let Ok(layers) = shared_layers.lock() {
                ...
                if resolution_changed || diff >= full_redraw_threshold {
                    // Full copy straight into the GPU staging image
                    full_img.bytes[..expected_len].copy_from_slice(&layer.pixels[..expected_len]);
                    full_redraw_needed = true;
                } else if diff > 0 {
                    // Fast delta copies direct to local memory
                ...
                if resolution_changed {
                    ...
                    smooth_head_pos += actual_total_updates as f64 - prev_mode_updates as f64;
        ...
        // Apply updated shader textures
        shader_material.set_texture("colormap", colormap_texture.clone());
        shader_material.set_texture("y_mapping", y_mapping_texture.clone());
        ...
        // --- OPTIMIZATION 3: MANUAL FRAME PACING ---
        let min_frame_time = 1.0 / 60.0;
