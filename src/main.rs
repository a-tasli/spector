...
fn draw_ui_overlay(mel: bool, cmap: ColorMapType, dir: ScrollDirection, fft: usize, history: usize) {
    ...
    for stat in stats {
        ...
        // Draw underline for hotkey
        if let Some(c) = stat.hotkey {
            // Primitive way to find the character position, assuming the hotkey is the first char
        ...
        if is_vertical {
            ...
            cursor_x += label_width + val_width + 25.0; // Fixed spacing
        }
    }
}